...
void sensorDataCallback(const beginner_tutorials::SensorData::ConstPtr& msg)
{
  ...
  dt = ((curTime - lastTime) % (int)10e9 ) * 10e-9;
}

void pixXYCallback(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
  pixX = msg->pose.position.x;
  pixY = msg->pose.position.y;
}
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber pixXY = n.subscribe("/mavros/local_position_tmp/pose", 100, pixXYCallback);
  ros::Publisher posStates = n.advertise<geometry_msgs::Point>("pos_states",100);
  
  ros::Rate loop_rate(100); // 100 Hz

  // Check params to see if writing to file!
  int writeData;
  n.getParam("/pos_sse_writeData", writeData);
  ...
  // Initialize for previous states (for Euler)
  ...
  while(ros::ok){
    ...
    if (newData) {
      ...
      // Here we need to recreate the entire YBu matrix
        ...
        shiftArray(Y, numSensors, timeSteps);
        for(size_t i = 0; i < numSensors; ++i){
          Y[(timeSteps-1)*numSensors + i] = pos[i];
        }
      ...
      // printf("Y: \n");
      // printArrayDouble(Y,numSensors,timeSteps);

      // printf("U: \n");
      // printArrayDouble(U,numInputs,timeSteps);

      // printf("Full YBu:\n");
      // printArrayDouble(YBu, numSensors, timeSteps);
      ...
      // printf("Optimized x BEFORE dynamics propagation is:\n");
      ...
      printf("x: \n");
      printArrayDouble(x, 1, numStates);
    ...
    // printf("Print CA\n");
    // printArrayDouble(CA, nonZeroEntries, timeSteps);
