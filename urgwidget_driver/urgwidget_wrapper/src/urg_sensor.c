...
{
    long try_baudrate[] = { 19200, 38400, 57600, 115200 };
    ...
    for (i = 0; i < try_times; ++i) {
        ...
        // printf("BAUDRATE: %d", try_baudrate[i]); // ROBERT CYPRUS INSERTED
                          ...
                          int scan_times, int skip_scan)
{
    ...
    case URG_DISTANCE_INTENSITY:
        ret = send_distance_command(urg, scan_times, skip_scan,
                                    'G', 'M', 'D'); // ROBERT CYPRUS, 'D' WAS 'E'
                                                    // THIS FIXED THE PROBLEM ZZZZZZZZZZZZZ
        break;
