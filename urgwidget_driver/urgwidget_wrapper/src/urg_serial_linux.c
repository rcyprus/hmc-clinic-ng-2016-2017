...
#include <fcntl.h>
#include <unistd.h>
#include <sys/select.h> // ROBERT CYPRUS INSERTED
...
int serial_write(urg_serial_t *serial, const char *data, int size)
{
    ...
    //printf("\nsize: %d: ", size); // ROBERT CYPRUS INSERTED
    //printf("write data: %s", data); // ROBERT CYPRUS INSERTED
                            ...
                            urg_serial_t* serial, int timeout)
{
    ...
    while (filled < data_size_max) {
        ...
        //printf("require_n: %d, ", require_n); // ROBERT CYPRUS INSERTED
        filled += read_n;
    }
    //printf("int_rcv filled: %d\n", filled); // ROBERT CYPRUS INSERTED
    return filled;
}
...
int serial_read(urg_serial_t *serial, char *data, int max_size, int timeout)
{
    ...
    filled += internal_receive(&data[filled], max_size - filled,
                               serial, timeout);
    //printf("data: %s", data); // ROBERT CYPRUS INSERTED
    return filled;
}
